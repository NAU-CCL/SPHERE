...
class ModelFactory:
    """
    The model factory class is used to create the model instances.
    ...
    def list_model_types() -> None:
        ...
        for model_type, (param_cls, _, stochastic, _) in (
                ModelFactory.MODEL_TYPES.items()):
    ...
    @staticmethod
    def create_model(model_type: str, params: Parameters, solver: Solver):
        """Create an instance of a model.

        Args:
            model_type: The type of model to create.
            params: The parameters for the model.
            solver: The solver to use for the model.
        ...
        if model_type not in ModelFactory.MODEL_TYPES:
            raise ValueError(f"Unknown model type: {model_type}. Use ModelFactory.list_model_types() to see available types.")

        param_cls, det_sir_cls, stoch_sir_cls, output_cls = ModelFactory.MODEL_TYPES[model_type]

        if not isinstance(params, param_cls):
            raise TypeError(f"Expected {param_cls.__name__} for {model_type} model.")

        if model_type == "SIR":
            if isinstance(solver, DeterministicSolver):
                transition = det_sir_cls(params)
            elif isinstance(solver, StochasticSolver):
                transition = stoch_sir_cls(params)
            else:
                raise ValueError(f"{solver} is not compatible with the SIR system.")
        elif model_type == "Lorenz63":
            if not isinstance(solver, DeterministicSolver):
                raise ValueError(f"{solver} is not compatible with the Lorenz 63 system.")
